...
export default function AccountWidget() {
  ...
  useEffect(() => { const saved = localStorage.getItem("ocean-guide-user"); if (saved) setUser(JSON.parse(saved)); }, []);
  ...
  useEffect(() => {
    ...
    withdraw.onclick = () => { if (!window.confirm("계정을 탈퇴하시겠습니까? 저장된 데모 계정이 삭제됩니다.")) return; localStorage.removeItem("ocean-guide-user"); localStorage.removeItem("ocean-guide-last-email"); setUser(null); setOpen(null); window.dispatchEvent(new Event("ocean-auth-changed")); };
  ...
  const submit = (e: FormEvent) => { e.preventDefault();
    ...
    if (open === "signup") { const next = { email, name: name || email.split("@")[0], password, points: 0 }; localStorage.setItem("ocean-guide-user", JSON.stringify(next)); localStorage.setItem("ocean-guide-last-email", email); setUser(next); window.dispatchEvent(new Event("ocean-auth-changed")); setMessage("가입이 완료되었습니다."); return; }
    const saved = localStorage.getItem("ocean-guide-user"); const found = saved ? JSON.parse(saved) as User : null;
    if (found && found.email === email && found.password === password) { localStorage.setItem("ocean-guide-last-email", email); setUser(found); window.dispatchEvent(new Event("ocean-auth-changed")); setMessage("로그인되었습니다."); } else setMessage("이메일 또는 비밀번호를 확인해주세요.");
  };
