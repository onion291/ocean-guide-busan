...
function ReportView() {
  ...
  const analyze = (type: string) => { const filename = inputRef.current?.files?.[0]?.name.toLowerCase() || ""; const inferred = type === "플라스틱" && filename ? (/(망|net|rope)/i.test(filename) ? "폐어망" : /(foam|스티로폼)/i.test(filename) ? "스티로폼" : /(can|캔)/i.test(filename) ? "캔" : /(glass|유리)/i.test(filename) ? "유리병" : "플라스틱") : type; const detected = reportKind === "유해 해양생물" && inferred === "플라스틱" ? "해파리 출몰" : inferred; setSample(detected); setResult(false); setAnalyzing(true); setTimeout(() => { setAnalyzing(false); setResult(true); }, 1450); };
