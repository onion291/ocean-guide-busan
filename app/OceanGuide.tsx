...
function EcoView() {
  ...
  useEffect(() => { const load = () => { const raw = localStorage.getItem("ocean-guide-user"); if (!raw) { setAuth(null); return; } const parsed = JSON.parse(raw); if (parsed.name === "김해린" || String(parsed.name || "").includes("해린")) { localStorage.removeItem("ocean-guide-user"); setAuth(null); return; } setAuth(parsed); }; load(); window.addEventListener("ocean-auth-changed", load); return () => window.removeEventListener("ocean-auth-changed", load); }, []);
