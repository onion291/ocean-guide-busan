...
function EcoView() {
  ...
  useEffect(() => { const load = () => { const raw = localStorage.getItem("ocean-guide-user"); setAuth(raw ? JSON.parse(raw) : null); }; load(); window.addEventListener("ocean-auth-changed", load); return () => window.removeEventListener("ocean-auth-changed", load); }, []);
  useEffect(() => { if (!auth) return; const hero = document.querySelector(".profile-hero"); if (!hero) return; const name = auth.name || auth.email.split("@")[0]; const avatar = hero.querySelector<HTMLElement>(".big-avatar"); if (avatar) avatar.childNodes[0].textContent = name.charAt(0).toUpperCase(); const heading = hero.querySelector<HTMLElement>(".profile-main h1"); if (heading) heading.textContent = `${name} 님`; const point = hero.querySelector<HTMLElement>(".point-card strong"); if (point) point.innerHTML = `${(auth.points || 0).toLocaleString()}<small>P</small>`; }, [auth]);
