...
function BeachView({ openMap }: { openMap: () => void }) {
  ...
  useEffect(() => { const stats = document.querySelectorAll<HTMLElement>(".hero-stats b"); if (stats[0]) stats[0].textContent = String(beaches.filter((b) => b.safety === "안전").length); if (stats[1]) stats[1].textContent = String(beaches.filter((b) => b.safety !== "안전").length); if (stats[2]) stats[2].textContent = kmaTemp ? `${kmaTemp}°` : "--"; const labels = document.querySelectorAll<HTMLElement>(".hero-stats span"); if (labels[2]) labels[2].textContent = "현재 기온"; }, [kmaTemp]);
